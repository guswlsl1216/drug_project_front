        ...
        {isLoggedIn ? (
          <form onSubmit={handleQuestionSubmit} className="question-form">
            ...
            <p className="login-prompt">
              상품에 대해 궁금한 점이 있으신가요? <span className="text-primary" onClick={() => goTo("/login")} >로그인</span> 후 질문을 등록해주세요.
            </p>
          )}
      </div>
            ...
            {qnaList.map((qna) => (
              <li key={qna.id} className={`qna-item ${qna.answer ? 'answered' : 'pending'}`}>
                ...
                <div className="qna-content">
                  {(qna.is_private && !isQuestioner(qna.user_nickname))
                    ? "작성자 또는 관리자만 확인할 수 있는 비밀글입니다."
                    : qna.question
                  }
                </div>
