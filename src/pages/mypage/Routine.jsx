  ...
  return (
    <>
      <div className="wrapper">
        <section className="routine_bg">
          <div className="bg_text">
            <h1 className="bg_main_title">Routine</h1>
            <p className="bg_sub_title">규칙적으로 약을 복용하세요.</p>
          </div>
        </section>
        <div className="routine-page">
          <p className="routine-sub-text">
            &#8251; 하루의 모든 루틴을 완료하면 <strong>10포인트</strong>를 드립니다! (매일 0시 지급)
          </p>
          <p className="routine-sub-text2">
            &#8251; <strong>당일</strong>에만 체크할 수 있습니다.
          </p>
            ...
            <div className="routine-calendar-section">
              <FullCalendar
                ...
                dayCellClassNames={(arg) => {
                  ...
                  events.map((data) => {
                    if (
                      new Date(now) >= new Date(data["start"]) &&
                      new Date(now) <= new Date(data["end"])
                    ) {
                      outofrange = false;
                  ...
                  if (classes.includes("warning")) {
                    ...
                    classes.filter((el) => el === "danger").length +
                      classes.filter((el) => el === undefined).length ==
                    classes.length
                  ) {
                    classname = "danger";
                  } else if (
                    classes.filter((el) => el === "good").length ==
                    classes.length -
                      events.filter(
                        (el) =>
                          new Date(el["start"]) > new Date(now) ||
                          new Date(el["end"]) < new Date(now)
                      ).length
                  ) {
                    classname = "good";
                  } else {
