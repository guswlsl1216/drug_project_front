...
이 프로젝트는 YOLO 기반 약품 이미지 감지를 통한 분석, 악플 감지 모델을 활용한 댓글 필터링, 유저 중심의 UI/UX(루틴, 쇼핑)를 결합하여 안전한 복용 관리에 필요한 기능을 한 곳에서 제공하는 것을 목표로 합니다.

### 주요 기능 요약
AI 사진 인식을 통해 복용 중인 의약품을 검색하고, 의약품끼리 혹은 영양제와의 비교 분석을 통해 상호작용과 위험 요소를 파악할 수 있습니다. 

사이트 내 영양제 스토어 이용 시에도 복용약과의 상호작용을 분석하여 안전하게 구매할 수 있습니다. 악성 댓글 모델을 이용한 필터링 기능으로 불필요한 정보를 막을 수 있습니다.
...
- 회원가입 : 전화번호와 주소는 선택사항이지만, 미리 작성해 두면 상품 주문 시 수월하게 배송지 정보를 입력할 수 있어요.
(gif)
...
3. 결과 보기 버튼을 누르면 의약품-영양제 간 상호작용 분석 결과를 알 수 있어요. 돋보기 버튼을 누르면 분석된 의약품과 영양제의 상세정보를 볼 수 있어요.
(gif)

4. 저장한 결과는 분석 결과 내역에서 확인할 수 있어요.
(gif)
</div>
</details>
...
- Medi.Check!에서 판매하는 영양제들을 구경해 보세요.
(gif)

- 내가 고른 제품이 복용 중인 약과 함께 섭취해도 괜찮은지 확인해 보세요.
(gif)

- 마음에 드는 제품을 찜해 보세요.
(gif)

- 장바구니와 바로구매, 원하는 방식으로 결제할 수 있어요. Medi.Check!는 토스페이를 비롯한 다양한 결제 방식을 지원하고 있어요.
(gif)

- 구매한 제품에 대한 리뷰를 남겨 보세요. 악플은 자동으로 감지되어 필터링됩니다.
(gif)
...
우측 하단의 챗봇 메뉴를 눌러 챗봇을 사용해 보세요. OpenAI가 의약품과 영양제 정보를 자세하게 알려줘요.
(gif)
</div>
</details>
...
- 상품 관리 : 관리자는 상품 등록, 수정, 품절 관리를 할 수 있어요.
- 상품 등록
(gif)
...
- 문의 내역 관리 : 상품 문의와 고객센터 문의를 관리할 수 있어요.
- 상품 문의
(gif)
...
- **강미선** : YOLO모델 전처리 및 학습, 상품 목록, 상세 페이지, 찜 목록, 문의사항 및 QnA, 푸터
- **배연희** : 챗봇, 스토어 관리자 기능, 주문, api 데이터 DB 저장, 공통 컴포넌트 설계
- **송상윤** : 악성 댓글 모델 학습, 루틴 캘린더, 스토어 리뷰
- **장수현** : YOLO모델 전처리 및 학습, 분석 결과 및 내역, 결제, 스토어 퀵메뉴, 헤더
- **진현진** : 악성 댓글 모델 크롤링 및 전처리, 학습, 루틴 관리/저장
- **한소연** : 회원 기능(회원가입/로그인 등), 스토어 장바구니
